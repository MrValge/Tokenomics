...
contract LGE is Ownable, LivePrice {
    ...
    constructor() {
      ...
      decimals = 10**18;
      maxMintableTKN = decimals.mul(5000000);
    ...
    function endLGE(uint256 _swapSlippage) public onlyOwner {
      ...
      uint256 amountTKN = amountOut.mul(5);
      if(amountTKN > maxMintableTKN) amountTKN = maxMintableTKN;
      TKN.mint(address(this), amountTKN);
      TKN.approve(address(UniSwap), amountTKN);
      STABLE.approve(address(UniSwap), amountSTABLE);
      (, , uint256 amountLP) = UniSwap.addLiquidity(address(TKN), address(STABLE), amountTKN, amountSTABLE, amountTKN.mul(80).div(100), amountSTABLE.mul(80).div(100), address(this), block.timestamp.add(180));
      address liquidityToken = Factory.getPair(address(TKN), address(STABLE));
    ...
    function pushInterfaceAddresses(address _TKN, address _STABLE, address _liquidity, address _balancerTrader) public onlyOwner {
      TKN = TKNInterface(_TKN);
